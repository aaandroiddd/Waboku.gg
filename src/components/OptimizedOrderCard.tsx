...
interface OptimizedOrderCardProps {
  order: Order;
  isSale?: boolean;
  userName?: string; // Pre-fetched user name to avoid individual API calls
}

export const OptimizedOrderCard = memo(function OptimizedOrderCard({ 
  order, 
  isSale = false, 
  userName 
}: OptimizedOrderCardProps) {
          ...
          <div className="flex-1 md:min-h-0">
            ...
            <div className="space-y-2">
              <p className="text-muted-foreground">
                {isSale ? 'Buyer: ' : 'Seller: '}
                {userName ? (
                  <UserNameLink userId={isSale ? safeOrder.buyerId : safeOrder.sellerId} fallbackName={userName} />
                ) : (
                  <UserNameLink userId={isSale ? safeOrder.buyerId : safeOrder.sellerId} fallbackName="Loading..." />
                )}
              </p>
              <p className="text-muted-foreground">
