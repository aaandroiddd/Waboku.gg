...
export function ShippingInfoDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update the offer with shipping information
      const { db } = getFirebaseServices();
      const offerRef = doc(db, 'offers', offerId);
      
      await updateDoc(offerRef, {
        shippingAddress,
        shippingInfoProvided: true,
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Your offer for "{listingTitle}" has been accepted! Please provide your shipping address to complete the transaction.
          </DialogDescription>
        </DialogHeader>
