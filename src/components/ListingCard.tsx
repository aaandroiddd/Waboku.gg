...
import { MobileAnimationWrapper, MobileMotionSpan } from './MobileAnimationWrapper';
...
const ListingCardContent = memo(({ listing, isFavorite, onFavoriteClick, getConditionColor, searchTerm, resultPosition, imageOnly = false }: ListingCardProps) => {
  ...
  const calculatedDistance = useMemo(() => {
    ...
    return null;
  }, [location?.latitude, location?.longitude, listing?.location?.latitude, listing?.location?.longitude]);
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      ...
      try {
        ...
        } else if (listing.expiresAt && typeof (listing.expiresAt as any)?.toDate === 'function') {
          expiresAtDate = (listing.expiresAt as any).toDate();
        } else if (listing.expiresAt && typeof listing.expiresAt === 'object' && 'seconds' in (listing.expiresAt as any)) {
  ...
  useEffect(() => {
    ...
    if (checkedListingsArray.includes(listing.id)) return;
    ...
    if (new Date() > expiresAt) {
      ...
      fetch('/api/listings/check-expiration', {
        ...
        .catch(err => console.error('Background expiration check failed:', err))
        .finally(() => {
          if (isMounted) setIsCheckingExpiration(false);
        });
  ...
  }, [listing.id, listing.expiresAt, listing.status, isCheckingExpiration]);
  ...
  return (
    <MobileAnimationWrapper
      ...
      whileHover="hover"
    >
      <Card className="relative overflow-hidden group h-full transform-gpu">
        <Link
        ...
        >
          <CardContent className="p-3 h-full flex flex-col" style={{ minHeight: imageOnly ? undefined : '420px' }}>
            <div className="aspect-square bg-muted rounded-lg mb-4 relative overflow-hidden flex-shrink-0 transform-gpu">
              {!imageOnly && (
                <>
