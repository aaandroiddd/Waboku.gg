...
export function PickupQRCode({ order, isSeller, onPickupCompleted }: PickupQRCodeProps) {
  ...
  const handleCodeVerification = async () => {
    ...
    try {
      ...
      // Store order details in localStorage and redirect to confirmation page
      const orderDetails = {
        orderId: data.orderDetails.orderId,
        listingTitle: data.orderDetails.listingTitle,
        sellerName: data.orderDetails.sellerName,
        amount: data.orderDetails.amount,
        pickupToken: data.orderDetails.pickupToken
      };
      
      localStorage.setItem(`pickup_confirmation_${data.orderDetails.orderId}`, JSON.stringify(orderDetails));
      
      setShowScanDialog(false);
      toast.success('Pickup code verified! Redirecting to confirmation page...');
      
      // Redirect to confirmation page
      window.location.href = `/pickup/confirm/${data.orderDetails.orderId}`;
  ...
  const processQRScan = async (qrData: string) => {
    ...
    try {
      ...
      setShowScanDialog(false);
      toast.success('QR code verified! Redirecting to confirmation page...');
